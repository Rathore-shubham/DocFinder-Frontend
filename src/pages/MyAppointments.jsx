...
const MyAppointments = () => {
  ...
  const getUserAppointments = async () => {
    try {
      const { data } = await axios.get(
        "https://DocFinder-backend-1af3.onrender.com/api/user/appointments",
        { headers: { token } }
      );
  ...
  const cancelAppointment = async (appointmentId) => {
    try {
      const { data } = await axios.post(
        "https://DocFinder-backend-1af3.onrender.com/api/user/cancel-appointment",
        { appointmentId },
        { headers: { token } }
      );
  ...
  const appointmentStripe = async (appointmentId) => {
    try {
      const { data } = await axios.post(
        "https://DocFinder-backend-1af3.onrender.com/api/user/payment-stripe",
        { appointmentId },
        { headers: { token } }
      );
      if (data.success) {
